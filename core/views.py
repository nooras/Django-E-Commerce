...
class CheckoutView(View):
    def get(self, *args, **kwargs):
        try :
            ...
            # orderr = Order.objects.get(user=self.request.user, ordered=False)
            # print(orderr.items, "JJBJBJBb")
            form = CheckoutForm()
            context = {
                'form' : form,
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    if address_qs.exists():
                        ...
                        message.info(self.request, "No default shipping address available.")
                        return redirect('core:checkout')
                else:
                ...
                if same_billing_address:
                    ...
                    if is_valid_form([billing_address1, billing_address2, billing_zip]):
                        ...
                        message.info(self.request, "Please fill in the required Billing address fields")
            ...
            messages.warning(self.request, "You dont have active order")
...
class PaymentView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                #For updating order after buy or after removing
    ...
    #     except stripe.error.StripeError as e:
    #         messages.error(self.request, "Something went wrong. Try again after some time.")
    #         return redirect("/")

    #     except Exception as e:
    #         messages.error(self.request, "A serioe error occured...")
    #         return redirect("/")
    #     messages.warning(self.request, "Invalid data received")
    #     return redirect("/payment/stripe/")
...
def add_to_cart(request, slug):
    ...
    # order_qs = Order.objects.filter(user=request.user, ordered=False)
    # #order_qs = Order.objects.filter(user=request.user, ordered=False)
    # print('orderouttt: ', order_item.quantity)
    # if order_qs.exists():
    #     order = order_qs[0]
    #     if order.items.filter(item__slug=item.slug).exists():
    #         print('orderAdd : ', order_item.quantity)
    #         order_item.quantity += 1
    #         order_item.save()
    #         messages.info(request, "This item quantity was updated.")
    ...
    #         return redirect('core:order-summary')
    # else:
    #     print('orderAdd else : ', order_item.quantity)
    #     #ordered_date = timezone.now()
    #     #ordered_date = timezone.now()
    #     ordered_date = timezone.now()
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            print(order_item.quantity, order_item,"ORDERRR : " ,order)
...
def get_coupon(request, code):
    try :
        ...
        messages.info(request, "This coupon doesnt exist.")
        return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                print(get_coupon(self.request, code), "hhhhhh")
                if coupon:
                    order.coupon = coupon
                    order.save()
                    ...
                    messages.warning(self.request, "Coupon code is invalid.")
                    return redirect("core:checkout")
            except ObjectDoesNotExist:
                messages.info(self.request, "You dont have an active order.")
                return redirect("core:checkout")
